    ...
    for filename in sorted(filenames):
        if filename.endswith(suffix):
            if first not in filename:
                call('somoclu -k 2 -x 25 -y 15 -c ' + previous + out_suffix + ' -l 0.05 -e 3 -t exponential -m toroid -r 5 ' + filename + ' ' + filename + this_suffix, shell=True)
                # print 'somoclu -k 2 -x 25 -y 15 -c ' + previous + out_suffix + ' -l 0.05 ' + filename + ' ' + filename + this_suffix
            else:
                call('somoclu -k 2 -x 25 -y 15 -c ' + initialcb + ' -l 0.05 -e 3 -t exponential -m toroid -r 5 ' + filename + ' ' + filename + this_suffix, shell=True)
